...
TOKEN = '7445992770:AAFcK3pmKtqd70QdjhP8mxe1112ZIe4TyZw'
    ...
    if action == '/approve':
        if plan == 1:  # Instant Plan 🧡
            if users_collection.count_documents({"plan": 1}) >= 99:
                bot.send_message(chat_id, "*Approval failed: Instant Plan 🧡 limit reached (99 users).@raj14754*", parse_mode='Markdown')
                return
        elif plan == 2:  # Instant++ Plan 💥
            if users_collection.count_documents({"plan": 2}) >= 499:
                bot.send_message(chat_id, "*Approval failed: Instant++ Plan 💥 limit reached (499 users).@raj14754*", parse_mode='Markdown')
                return
...
def handle_message(message):
    if message.text == "Instant Plan 🧡":
        ...
        bot.send_message(message.chat.id, "*Please use the following link for Canary Download: https://t.me/c/2037296029/11208*", parse_mode='Markdown')
